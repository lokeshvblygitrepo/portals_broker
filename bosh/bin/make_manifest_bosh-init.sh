...
- name: bosh
  ...
  properties:
    ...
    agent: {mbus: "nats://nats:$BOSH_PASSWORD@$STATC_IP:4222"}
...
cloud_provider:
  ...
  ssh_tunnel:
    host: $STATIC_IP # <--- Replace with your Elastic IP address
    port: 22
    user: vcap
    private_key: $PRIVATE_KEY_PATH # Path relative to this manifest file

  mbus: "https://mbus:$BOSH_PASSWORD@$STATIC_IP:6868" # <--- Replace with Elastic IP
